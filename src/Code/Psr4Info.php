...
final class Psr4Info implements ClassInfo
{
    ...
    private function normalizeNamespace(string $namespace): string
    {
        ...
        \array_walk($splitNamespace, function (&$item) {
            $item = \ucfirst($item);
        });

        return \trim(\implode('\\', $splitNamespace), '\\');
    }
    ...
    {
        $path = \preg_replace('/^' . \addcslashes($this->sourceFolder, '/') . '\//', '', $path);
        $splitPath = \explode('/', $path);

        \array_walk($splitPath, static function (&$item) {
            $item = \ucfirst($item);
        });
